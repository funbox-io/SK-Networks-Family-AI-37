...
from common import ROOT, SITE, load_schedule, scan_days, scan_weekly  # noqa: E402
...
def build():
    ...
    A("| 📖 **설정 · 사용법** | [.setup/SETUP.md](.setup/SETUP.md) |")
    A("")
    A("---\n")
    ...
    A("## 🛠 이 저장소 쓰는 법\n")
    A("```bash")
    A("python3 .setup/scripts/new_day.py              # 오늘 수업일 폴더 만들기")
    A("python3 .setup/scripts/new_day.py 2026-09-14   # 특정 날짜로 만들기")
    A("python3 .setup/scripts/new_day.py --next       # 아직 안 만든 가장 이른 수업일")
    A("python3 .setup/scripts/new_day.py --weekly     # 이번 주 회고 파일 만들기")
    A("python3 .setup/scripts/build_index.py          # 진도표만 다시 만들기")
    A("```\n")
    ...
    A(f"<sub>이 문서는 <code>.setup/scripts/build_index.py</code> 가 자동으로 만듭니다. "
    ...
    print(f"README.md · .setup/site/data.json 갱신 완료 — {len(done)}/{total}일 기록됨")
